                ...
                ANN(0), qrsANN(0), AUX(0)
{
        if (p) {
                ...
                ahdr.qrsFreq = 10.0;    //QRS filtration frequency  13 default
                ahdr.ampQRS = 1;   //inter1 filter
...
void EcgAnnotation::GetEctopics(int **ann, int qrsnum, double sr, vector<double> *RRtemps, vector<double> *QRtemps) const
{
		...
		for (int n = 0; n < qrsnum - 1; n++){ 
                RRs.push_back((double)(ann[n*2+2][0] - ann[n*2][0]) / sr);//qrsNum-1 rr's
				 QRS.push_back((double)(ann[n * 2+1][0] - ann[2 * n][0])/sr); 
			//	wprintf(L"n %d n*2 %d n*2+1 %d qrs %f\n", n, ann[n * 2][0],ann[n*2+1][0],QRS[n]);
				 int msec = int(((double)ann[2*n][0] / sr) * 1000.0);
				if (QRS[n]>0.12){
					ann[n * 2 ][1] = 46;
					
				}
		//		wprintf(L"alla rrs %f QRS:%f n %d\n", RRs[n],QRS[n], n);
			...
			//		wprintf(L"k %d  n %d\n", k,n);
					// standardavvikelse
					if (QRS[n] <= 0.120)// finds normal beats
					{
						d++;
						numb.push_back(n);
						//sum_rr = sum_rr + RRs[n];
						sum_rr = ((sum_rr*(d - 1)) + RRs[numb[T]]) / d;
					//	wprintf(L"RR:%f T:%d Sum:%f n:%d msec%d \n",RRs[numb[T]],T,sum_rr, n,msec);
						T++;
			...
			//		wprintf(L"sum_rr:%f \n", sum_rr);
				...
				if (n == k){
...

					{
						int nn = (T-1) - t;
						int g = numb[nn];
					//	wprintf(L"nnb:%d  value:%d\n", nn, numb[nn]);
						SD = (SD + pow((RRs[g] - mean_rr), 2));
					//	wprintf(L"rrs=%f mean:%f  rr-me:%f ^2: %f sum:%f T:%d \n" , RRs[nn], mean_rr, RRs[nn] - mean_rr, pow((RRs[nn] - mean_rr), 2),SD,T);
				//		wprintf(L"SD:%f\n",SD);
					...
					if (n==k)
					{
						...
						for (int o = n; o>=(b+1); o--)
						{
							...
							if (RRs[n - o] >(mean_rr + SD) || RRs[n - o] < (mean_rr - SD))
							{ 
								h++;
								wprintf(L" n:%d avvikelse %f  msec%d antal:%d\n", n, RRs[n - o], int(((double)ann[2 *( n-o)][0] / sr) * 1000.0), h);
							}
						}
						a = msec;
						b = n;
						...
						{	
							...
							numb.push_back(n);				
							sum_rr = ((sum_rr*(d - 1)) + RRs[numb[T]]) / d;
						//	wprintf(L"RR2:%f  T:%d sum:%f\n",RRs[numb[T]], numb[T],sum_rr);
							T++;
						}
				...
				// HRV looks for the mean RR-distans for the past 60 beats, if extra beats less than 0,75% of mean ectopisk beat
		/*		if (n < 60){

					k++;
					sum_rr = ((sum_rr*(k - 1)) + RRs[n]) / k;
					mean_rr = sum_rr;

				//	wprintf(L" mean %f   rr %f\n", sum_rr, RRs[n]);
				}
					*/
		...
		//		}
				...
				/*	if (n>1){
						 ...
						 SD = SD+pow((RRs[n] - mean_rr), 2);
						 SD = sqrt(SD / n);
						 double dif = mean_rr - SD;

						wprintf(L"SD: %f  : %d   dif:%f rr_rr:%f\n", SD,n,dif,mean_rr);
					}*/
				
		/////////////////////////////////////////////////////////////////////////////////////////////////				
		/*				if ((RRs[n] >= RRs[n - 1] * 0.85) && QRS[n] < 0.120 && (RRs[n] <= 1.5)){
							k++;
							sum_rr = (sum_rr + RRs[n]);
							mean_rr = sum_rr /k;
							//wprintf(L"RR-mean %f \n", mean_rr);
				...
				//	wprintf(L"RR-mean dif %f \n", abs(RRs[n] - mean_rr));

					if ((abs(RRs[n] - mean_rr))> max){
						max = abs(RRs[n] - mean_rr);
					}
						 
					if ((abs(RRs[n] - mean_rr)!=0) && (abs(RRs[n] - mean_rr))<min) { min = abs(RRs[n] - mean_rr); }
				} */
				//wprintf(L"min %f  max: %f \n", min,max); 

		/*		if (n > 60){ 
				//	if ((RRs[n] >= RRs[n - 1] * 0.85) && QRS[n] < 0.120 && (RRs[n] <= 1.5))
					//	d++;
					
					sum_rr = ((sum_rr*60) - RRs[n - 60] + RRs[n]) / 60;
					mean_rr = sum_rr;
				} */

		//		if (n < 800)	{ wprintf(L"  rr %f  \n", RRs[n]); }


			//	k++;
				if (n>0 && RRs[n] < (mean_rr * 0.75) && RRs[n]*1.15 < RRs[n-1]){
					//	wprintf(L"RR-mean %f \n", mean_rr*0.75);
					//	wprintf(L"RR-1 %f \n", rr1);
					ann[n * 2 + 2][1] = 20;
					continue;
				...
				if (n>0 && RRs[n] > RRs[n-1]*1.25 && RRs[n]> mean_rr && RRs[n] < mean_rr*1.4) {
					ann[n * 2 + 2][1] = 21;
					continue;	
				}
				if (n>2 && fabs(RRs[n - 2] - RRs[n - 1]) < 0.3 && RRs[n - 2]< 0.8 && RRs[n - 1]< 0.8 && RRs[n] > 1.2*(RRs[n - 2] + RRs[n - 1]) && RRs[n]>mean_rr *1.65) {//2.4
					ann[n * 2 + 2][1] = 46;
					continue;
				}

			//	wprintf(L"RR-mean %f \n", mean_rr);
				/*if (RRs[n] > mean_rr*1.4) {
					ann[n * 2 + 2][1] = 46;
					continue;
				}*/
				
			
				if ( RRs[n] > mean_rr * 1.76){
					//	wprintf(L"RR %f \n", RRs[n]);
					//	wprintf(L"RR-1 %f \n", rr1);
						ann[n * 2 + 2][1] = 3;
					continue;
	...
	/////////////////////////////////////////////////////////////////////////////////////				
		...
		/*for (int n = 0; n < (qrsnum - 1); n++){
				...
				d++;
				SD = (SD + pow((RRs[n] - mean_rr), 2))/(d);	
				double dif = mean_rr - RRs[n];

			

			//	wprintf(L"  : %d   dif:%f rr_rr:%f\n", n, dif, mean_rr);	
		} */
	...
	//	max = mean_rr + (SD*2);
	//	min = mean_rr - (SD*2);
	//	wprintf(L"SD: %f   mean:%f  MAX:%f   min: %f \n", SD, mean_rr, max, min);
		//wprintf(L"RR-mean %f \n", mean_rr);
		...
		//	wprintf(L"int %d \n", RRs.size());
                ...
                if (60.0 / rr3 < ahdr.minbpm || 60.0 / rr3 > ahdr.maxbpm) //if RR's within 40-200bpm
                        continue;
			//	if (n >= 0 && QRS[n] > 0.12) { ann[n * 2 + 2][1] = 0;  wprintf(L"qrs %f \n", QRS[n]); }
				
	/*			if (n >= 1)
				{
					if (RRs[n] * 1.15 <= RRs[n - 1] || (RRs[n] * 1.15>=RRs[n - 1] && RRs[n]<min))
...
// orginal HRV 
     /*         if (1.15*rr2 < rr1 && 1.15*rr2 < rr3)
	   ...
	   ////////////////// sudden change in the rytm////////////////////////
		...
		/*		if (fabs(rr1 - rr2) < 0.3 && rr1 < 0.8 && rr2 < 0.8 && rr3 > 1.15*(rr1 + rr2)) {
					ann[n * 2 + 4][1] = 46;
					continue;
				}*/

                if (fabs(rr1 - rr2) < 0.3 && rr1 < 0.8 && rr2 < 0.8 && rr3 > 2.4*(rr1 + rr2)) {//2.4
                        ann[n*2+4][1] = 46;
                        continue;
                }
                ...
                ANN(0), qrsANN(0), AUX(0)
{
        if (p) {
                ...
                ahdr.minQRS = 0.04;     //min QRS duration
                ahdr.maxQRS = 0.2;      //max QRS duration
                ahdr.qrsFreq = 11.0;    //QRS filtration frequency
                ahdr.ampQRS = BIOR13;   //inter1 filter
                ahdr.minUmV = 0.2;      //min UmV of R,S peaks
                ahdr.minPQ = 0.07;      //min PQ duration
                ahdr.maxPQ = 0.20;      //max PQ duration
                ahdr.minQT = 0.21;      //min QT duration
...
int** EcgAnnotation::GetQRS(const double *data, int size, double sr, wchar_t *fltdir)
{
        ...
        if (fltdir) {
                ...
                //        return 0;
                //if(1)
                //if(Filter30hz(pdata,size,sr,fltdir) == false)
                        return 0;
        }
...
////////////////////////////////////////////////////////////////////////////////
// Find ectopic beats in HRV data
void EcgAnnotation::GetEctopics(int **ann, int qrsnum, double sr, vector<double> *RRtemps) const
{
        if (qrsnum < 3)
                return;
		//RRs->clear();
